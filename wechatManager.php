...
include(WECHAT_MANAGER_PLUGIN_DIR.'/wechat-manager-class.php');    // 微信类库  类似python的import，把其他5个文件包含进来
...
define("TOKEN",wm_get_setting('token'));  //TOKEN值   定义常量wm_get_setting在wechat_manager-functions.php中定义
define("POSTNUM",wm_get_setting('post_num'));  //TOKEN值 
$wm_thumb = wm_get_setting('thumb');           //定义变量
$wm_bdak = wm_get_setting('bd_key');
$wm_translate_appid = wm_get_setting('trans_appid');
$wm_translate_key = wm_get_setting('trans_key');

add_action('pre_get_posts', 'wm_preprocess', 4);   //add_action是wp的函数 参见http://wordpress.la/codex.html 不知道怎么导入的
function wm_preprocess($wp_query){
    global $object;
    if(!isset($object)){
        //创建一个WeChat类的实例, 回调函数名称为"onMessage",即消息处理函数
        $object = new WeChat(TOKEN, "onMessage");  //WeChat类在wechat-manager-class.php中定义，主要处理和微信服务器的交互
        $object->process();  //处理消息            //实际的处理通过回调方式由这里的OnMessage处理 
        return;
    }
}
